...
public class Util {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (zipEntry.getName().startsWith("lib/")) {
                ...
                if (supportArch.contains(arch) || supportArch.isEmpty()) {
                    ...
                    zos.putNextEntry(new ZipEntry("lib/armeabi/" + pathSegment.get(2)));
                    zos.write(IOUtils.toByteArray(zipFile.getInputStream(zipEntry)));
                }
            }
        }
    ...
    public static Set<String> zeldaSupportArch = new HashSet<>();

    public static void setupZeldaSupportArch(String settings) {
        zeldaSupportArch.addAll(Arrays.asList(settings.split(",")));
        if (zeldaSupportArch.contains("armeabi-v7a")) {
            zeldaSupportArch.add("armeabi");
        }
    }

    static boolean isZeldaUnSupportArch(String zipEntryName) {
        ...
        return !zeldaSupportArch.contains(nowArch);
    }
