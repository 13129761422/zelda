...
const char *relocate_path_internal(const char *path, int *result) {
    if (path == NULL) {
        *result = NOT_MATCH;
        return NULL;
    }
...
const char *relocate_path(const char *path, int *result) {
    const char *relocated_path = relocate_path_internal(path, result);
    if (relocated_path == nullptr) {
        return nullptr;
    }
    const char *begin_of_replace_index = strstr(relocated_path, origin_package_name);
    if (begin_of_replace_index == nullptr) {
        return relocated_path;
    }
    char *copy_buffer = static_cast<char *>(malloc(PATH_MAX));
    memset(copy_buffer, 0, PATH_MAX);


    // /data/data/com.tencent.mm/lib -> /data/data/zelda.comtencentmm.xxxx/lib

    //需要被替换
    size_t prefix_size = begin_of_replace_index - relocated_path;
    // /data/data/
    memcpy(copy_buffer, relocated_path, prefix_size);
    // /data/data/com.tencent.mm
    memcpy(copy_buffer + prefix_size, now_package_name, now_package_name_length);
    const char *suffix_copy_begin = begin_of_replace_index + origin_package_name_length;

    // /data/data/com.tencent.mm/lib
    memcpy(copy_buffer + prefix_size + now_package_name_length, suffix_copy_begin,
           strlen(relocated_path) - (suffix_copy_begin - relocated_path) + 1);
    FREE(relocated_path, path);
    return copy_buffer;
